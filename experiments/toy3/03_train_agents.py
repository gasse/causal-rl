...
if __name__ == '__main__':
    ...
    # ENVIRONMENT 
    from environments import TabularPOMDP, BeliefStateWrapper, RewardMapWrapper
    from agents import ActorCritic, train_agent
    ...
    gamma = 1
    n_epochs = 1000
    n_epochs_warmup = 200 # critic pretraining
    log_every = 10
    batch_size = 32
    ...
    for nsamples_obs in nsamples_obs_subsets:
        for nsamples_int in nsamples_int_subsets:
            for training_scheme in training_schemes:
                ...
                model_dir = pathlib.Path(f"experiments/toy3/trained_models/{privileged_policy}/seed_{seed}/nobs_{nsamples_obs}/nint_{nsamples_int}")
                agent_dir = pathlib.Path(f"experiments/toy3/trained_agents/{privileged_policy}/seed_{seed}/nobs_{nsamples_obs}/nint_{nsamples_int}")
